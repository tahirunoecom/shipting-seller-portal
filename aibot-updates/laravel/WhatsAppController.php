...
class WhatsAppController extends Controller
{
    ...
    public function saveSessionInfo(Request $request)
    {
        try {
			...
			// Auto-subscribe WABA to webhook
			if ($wabaId) {
				$this->subscribeWabaToWebhook($wabaId);
			}
	 ...
	 * This ensures new sellers' WABAs receive webhook events
	 */
	private function subscribeWabaToWebhook($wabaId)
	{
		$systemToken = config('services.meta.system_user_token');
		
		if (!$systemToken || !$wabaId) {
			Log::warning("Cannot subscribe webhook: missing token or WABA ID");
			return false;
		}
		
		try {
			Log::info("Subscribing WABA {$wabaId} to webhook using System User token");
			
			$response = Http::withToken($systemToken)
				->post("https://graph.facebook.com/v21.0/{$wabaId}/subscribed_apps");
			
			$result = $response->json();
			Log::info("Webhook subscription response: " . json_encode($result));
			
			if (isset($result['success']) && $result['success']) {
				Log::info("✅ Webhook subscribed successfully for WABA: {$wabaId}");
				return true;
			}
			
			Log::error("❌ Webhook subscription failed for WABA {$wabaId}: " . json_encode($result));
			return false;
